        ...
        <div className="flex gap-1 ml-auto">
          ...
          <button type="button"
            ...
            onClick={() => {
              ...
              if (playerState.state === 'playing') send(M.control(mp.id, 'pause')) // pause to set mark
          ...
          {(marks?.markIn != null || marks?.markOut != null) && (
            <button type="button"
              className="px-1.5 py-1 rounded text-[9px] text-zinc-600 border border-zinc-700 bg-transparent hover:text-white"
              onClick={() => send(M.setMarks(mp.id, playerState.currentFileIndex, undefined, undefined))}
              title="Clear marks">✕</button>
          )}
