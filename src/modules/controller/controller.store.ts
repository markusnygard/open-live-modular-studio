...
/** A source assignment resolved with its display name — mirrors the vision mixer input list. */
...
/** A DSK graphic assignment resolved with its graphic name. */
...
/** A macro definition — the execute-button data for the macro bar. */
...
interface ControllerState {
  ...
  // ─── Controller module additions ─────────────────────────────────────────────
  /** Resolved source assignments for the active production (mixer input list). */
  sources: SourceAssignment[]
  /** Resolved DSK graphic assignments for the active production. */
  graphicAssignments: GraphicAssignment[]
  /** Macro execute buttons for the active production. */
  macros: ControllerMacro[]
  /** Production config values (e.g. num_pips). */
  values: Record<string, string | number | boolean>
  /** Whether the controller options modal is open. */
  optionsOpen: boolean
  /** Transition chips visible in the transition panel. */
...
interface ControllerActions {
  ...
  setMacros: (macros: ControllerMacro[]) => void
  setOptionsOpen: (open: boolean) => void
  setVisibleTransitions: (transitions: string[]) => void
}

export const useControllerStore = create<ControllerState & ControllerActions>()(
  devtools(
    immer((set) => ({
      // State
      pgmInput: null,
      ...
      values: {},
      optionsOpen: false,
      visibleTransitions: ['fade', 'slide_left', 'slide_right'],
      ...
      setProductionData: (data) =>
        set((state) => {
          ...
          state.graphicAssignments = data.graphicAssignments
      ...
      setVisibleTransitions: (transitions) =>
        set((state) => {
          state.visibleTransitions = transitions
        }),
    })),
    { name: 'controller', enabled: import.meta.env.DEV },
  ),
)
