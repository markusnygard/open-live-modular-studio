...
/** Audio element descriptor as returned by `/api/v1/productions/{id}/audio`. */
  ...
  devtools(
    immer((_set, get) => ({
      ...
      applyLevel: (elementId, value) =>
        _set((s) => {
          s.levels[elementId] = Math.max(0, Math.min(10, value))
        }),

      applyMuted: (elementId, muted) =>
        _set((s) => {
          s.muted[elementId] = muted
        }),
      ...
      applyAfvByMixerInput: (mixerInput, enabled) => {
        ...
        if (el) {
          _set((s) => {
            s.afv[el.elementId] = enabled
          })
        } else {
          // Elements not loaded yet — queue and drain in setElements
          _set((s) => {
            s.pendingAfvByMixerInput[mixerInput] = enabled
          })
        }
      },
      ...
      applyDynamics: (channel, property, value) =>
        _set((s) => {
          s.dynamics[`ch${channel}_${property}`] = value
        }),
      ...
      toggleMute: (elementId) => {
        _set((s) => {
          s.muted[elementId] = !s.muted[elementId]
        })
      },

      toggleAfv: (elementId) =>
        _set((s) => {
          s.afv[elementId] = !s.afv[elementId]
        }),

      applyPfl: (elementId, enabled) =>
        _set((s) => {
          s.pfl[elementId] = enabled
        }),

      applyAfl: (elementId, enabled) =>
        _set((s) => {
          s.afl[elementId] = enabled
        }),
      ...
      togglePfl: (elementId) =>
        _set((s) => {
          s.pfl[elementId] = !s.pfl[elementId]
        }),
    })),
